...
const Navbar = () => {
    ...
    return (
        <header className="min-h-[5rem] w-full bg-background flex justify-between items-center p-4 px-6 border border-b border-primary border-t-0 border-l-0 border-r-0 ">
            ...
            {
                isAuthenticated && isAdmin ? <Button onClick={()=>router.push('/admin')} variant="default" className="bg-primary hover:bg-secondary text-text text-lg font-semibold " >Admin Dashboard</Button> :
                <Button onClick={()=>router.push('/login')} variant="default" className="bg-primary hover:bg-secondary text-text text-lg font-semibold " >Admin Login</Button>
            }
